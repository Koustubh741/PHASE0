#!/usr/bin/env python3
"""
Local Hugging Face Transformers Service for GRC Platform
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting Local Hugging Face Transformers Service...")
    
    # Set random seed for reproducibility
    set_seed(42)
    
    # Initialize shared resources in app.state
    app.state.loaded_models = {}
    app.state.model_pipelines = {}
    
    # Load default models
    await load_default_models(app.state)
    ...
    # Shutdown
    logger.info("Shutting down Local Hugging Face Transformers Service...")
    
    # Clean up models and free memory
    for model_name in list(app.state.loaded_models.keys()):
        try:
            if hasattr(app.state.loaded_models[model_name]["model"], 'cpu'):
                app.state.loaded_models[model_name]["model"].cpu()
            del app.state.loaded_models[model_name]
            del app.state.model_pipelines[model_name]
...
app = FastAPI(
    title="Local Hugging Face Transformers Service",
...
# Configuration
DEFAULT_MODEL = os.getenv("DEFAULT_MODEL_NAME", "ProsusAI/finbert")
EMBEDDING_MODEL = os.getenv("EMBEDDING_MODEL", "sentence-transformers/all-MiniLM-L6-v2")
MAX_MODEL_SIZE = os.getenv("MAX_MODEL_SIZE", "2GB")
ENABLE_GPU = os.getenv("ENABLE_GPU", "false").lower() == "true"
BFSI_TRAINING_DATA_SIZE = 25000

# Model allowlist for security - only these models can be loaded
ALLOWED_MODELS = {
    # Chat models
    ...
    # BFSI Model Stack - Enhanced Models
    "ProsusAI/finbert",
    ...
    "bfsi-compliance-model",
    "bfsi-finbert-model",
    "bfsi-distilbert-model",
    "bfsi-bart-model",
    "bfsi-bert-ner-model",
    "bfsi-qa-model",
    "bfsi-dialog-model",
...
async def load_default_models(app_state):
    """Load default models for the service"""
    try:
        logger.info(f"Loading default chat model: {DEFAULT_MODEL}")
        await load_chat_model(DEFAULT_MODEL, app_state)
        
        logger.info(f"Loading default embedding model: {EMBEDDING_MODEL}")
        await load_embedding_model(EMBEDDING_MODEL, app_state)
        
    except Exception as e:
        logger.error(f"Error loading default models: {e}")
        raise

async def load_chat_model(model_name: str, app_state):
    """Load a chat model"""
    try:
        # Security check: verify model is in allowlist
        if model_name not in ALLOWED_MODELS:
            error_msg = f"Model '{model_name}' is not in the allowlist. Only approved models can be loaded for security reasons."
            logger.warning(error_msg)
            raise HTTPException(status_code=403, detail=error_msg)
        
        if model_name in app_state.loaded_models:
        ...
        if tokenizer.pad_token is None:
            # Use EOS token as padding token if available, otherwise use a default
            if tokenizer.eos_token is not None:
                tokenizer.pad_token = tokenizer.eos_token
                logger.info(f"Set padding token to EOS token for model: {model_name}")
            else:
                # Fallback: add a new padding token
                tokenizer.add_special_tokens({'pad_token': '[PAD]'})
                logger.info(f"Added new padding token for model: {model_name}")
        ...
        # Store in app.state
        app_state.loaded_models[model_name] = {
            "model": model,
            "tokenizer": tokenizer,
            "type": "chat"
        }
        app_state.model_pipelines[model_name] = pipe
...
async def load_embedding_model(model_name: str, app_state):
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "service": "local-huggingface-transformers",
        "loaded_models": list(app.state.loaded_models.keys()),
...
async def list_models():
    ...
    models_info = []
    for model_name, model_data in app.state.loaded_models.items():
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        if request.model_name not in app.state.loaded_models:
            await load_chat_model(request.model_name, app.state)
        
        # Get the pipeline
        pipe = app.state.model_pipelines.get(request.model_name)
        if not pipe:
...
async def generate_embeddings(request: EmbeddingRequest):
    ...
    try:
        # Ensure model is loaded
        if request.model_name not in app.state.loaded_models:
            await load_embedding_model(request.model_name, app.state)
...
async def root():
    ...
    return {
        "service": "Local Hugging Face Transformers Service",
        "version": "1.0.0",
